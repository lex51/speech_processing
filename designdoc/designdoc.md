...
3. Возражения: были ли возражения у клиента и отработал ли их оператор.
4. Назначил ли оператор следующий шаг и определил ли срок.
5. Эмоциональный настрой и ясность диалога.

**Почему станет лучше, чем сейчас, от использования ML**: Использование машинного обучения позволит автоматизировать процесс оценки качества звонков, что приведет к уменьшению субъективности в оценках, снижению трудозатрат и увеличению скорости. Появится возможность охватывать 100% звонков (сейчас в бизнесе чаще используют подход веерной проверки - около 15% от всех звонков). Это позволит компаниям лучше понимать, как повысить качество обслуживания клиентов и обучить операторов.


**Что будем считать успехом с точки зрения бизнеса**: Работающая модель, предоставляющая оценку качества звонка по заданным критериям, а также отчет с более подробным анализом звонка.

Сейчас данную задачу для бизнеса решают внешние или внутренние асессоры: от 5 до 60 асессоров на одну компанию, в зависимости от объема звонков. При этом их покрытие звонков поставляет 15-20%. Модель обеспечит 100% покрытие звонков, сэкономит затраты на асессоров  и ускорит получение анализа работы операторов.
...
- Повышение охвата звонков до 100%
...
- Расширение функциональности системы для оценки других аспектов обслуживания клиентов, таких как скорость ответа на звонки, сравнение продолжительности звонков с другими операторами и т. д.
...
В скоуп проекта входит:

- Подготовка прототипа, представляющего собой чат-бот, который может принимать аудиофайлы и выдавать оценку звонка и развернутый анализ по заданным критериям.

В скоуп не входит:
- Создание сайта/приложения с пользовательским интерфейсом,
- Интеграция в CRM колл-центра.
...
- Распознавание речи и анализ текста диалога для оценки качества звонков
...
5. Аудиофайл транскрибируется и направляется в ChatGPT для анализа по заранее определенным критериям (по протестированным промптам).
6. Телеграмм бот получает результаты от моделей.
7. Телеграмм бот отправляет результаты пользователю через интерфейс бота.
...
Качество модели определения эмоций будем оценивать по метрикам:
...
Качество отработки промптов на ChatGPT будем оценивать по метрикам:
- Устойчивость и повторяемость ответов
- Сравнение с оценками асессоров
  
#### 3.2. Что считаем успешным пилотом  

Запущенный бот в Telegram, который при подаче в него аудиофайла выдает анализ звонка по заявленным критериям.
  
#### 3.3. Подготовка пилота  
***
Был проведен изначальный анализ данных на датасете [Dusha](https://github.com/salute-developers/golos/tree/master/dusha) в доменах "crowd" и "podcast". В процессе анализа были построены гистограммы, показывающие распределение звуковых файлов по различным эмоциям. Также была проведена проверка на наличие пропущенных значений, дубликатов и противоречивой информации. В результате была получена информация о качестве и точности данных в датасете.
![пример](dusha_eda.jpg)

Для проведения проверочного тестирования модели был собран небольшой набор данных из открытых источников, содержащий информацию о звонках, совершенных клиентами в разные службы технической поддержки.

***
Была опробована предобученная модель HubertForSequenceClassification на датасете Dusha. Подробности данного этапа можно найти в ноутбуке: https://github.com/lex51/speech_processing/blob/main/experiments/hubert-fine-tuned-on-dusha.ipynb.
  


#### 4. Подготовка демонстрационного стенда

*Пилотная версия для тестов и демонстрации возможностей системы потенциальному заказчику*

### 4.1 Архитектура демостенда

В качестве компонентов системы выбраны технологии:
- docker (docker-compose) для быстрого и удобного развертывания и управления контейнерами
- celery - классическое решение для управления очередями
- fastapi - простой rest-интерфейс для взаимодействия с моделью
- aiogram - библиотрека для написания телеграмбота с базовым функционалом

### 4.2 Описание инфраструктуры
- ubuntu 22.04 64, xeon E5 2620, 16 gb ddr4, nvme 250 gb, GeForce GTX 1060 5GB - выбор обусловлен наличием данных мощностей
> - к минусам можно отнести устаревшее оборудование
> - довольно долгая обаботка записей, примерно 5 мин записи в минуту
> - к плюсам можно отнести изначально заложенную возможность горизонтального масштабирования и быстрое развертывание новых инстансов воркера

### 4.3 Ограничения 
- большей частью производительность упирается в переполнение очереди и в длительность звуковых файлов, данный пункт на уточнении

#### 5. Подготовка production стенда

### 5.1 Архитектура production
*базовые компоненты перенесены из пилота*
- docker (docker-compose) для быстрого и удобного развертывания и управления контейнерами
- celery - классическое решение для управления очередями
- fastapi - простой rest-интерфейс для взаимодействия с моделью
***
- web-интерфейс для взаимодействия с системой и визуализации метрик (показателей) для конечного пользователя
- prometheus, grafana, sentry - мониторинг состояния серверов, визуализация метрик по нагрузке\операциям\доступности, сбор логов
- nginx - балансировка нагрузки

### 5.2 Описание инфраструктуры production стенда 
*данный пункт в разработке*

- несколько стендов на ускорителях уровня Tesla P100 или выше - основное требование
- к cpu, ram, rom требования еще не выработаны
- ubuntu with LTS

 
в качестве примера взят [design_doc](https://github.com/IrinaGoloshchapova/ml_system_design_doc_ru/blob/main/ML_System_Design_Doc_Template.md)
